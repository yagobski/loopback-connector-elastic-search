...
ESConnector.prototype.define = function(modelDefinition) {
  //console.trace('entering inside ESConnector.prototype.define'/*, 'modelDefinition', modelDefinition*/);
  log('ESConnector.prototype.define', 'start');
  var self = this;

  // call parent method
  ESConnector.super_.prototype.define.apply(self, arguments);

  // additional custom logic - this is why we hooked into this method
  self.setupMapping(modelDefinition.model.modelName)
      .then(function(){
        log('ESConnector.prototype.define', 'async flow finished');
      });
  log('ESConnector.prototype.define', 'sync flow finished');
};
...
ESConnector.prototype.connect = function (callback) {
    ...
    if (self.db) {
        ...
        // NOTE: any & all indices and mappings will be created (if needed) when the Model is defined
        process.nextTick(function () {
          callback && callback(null, self.db);
        });
    }
