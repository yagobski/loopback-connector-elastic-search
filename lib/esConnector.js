...
ESConnector.prototype.buildFilter = function (model, idName, criteria, size, offset) {
    ...
    if (criteria) {
        if (criteria.where) {
            filter.body = self.buildWhere(model, idName, criteria.where);
        }
        // TODO: Fields filter
        // TODO: Include filter
        // TODO: Limit filter
        ...
        // TODO: Skip filter
            ...
            log('ESConnector.prototype.buildFilter', 'invalid or unknown filter criteria');
            throw new Error('ESConnector.prototype.buildFilter: invalid or unknown filter criteria');
        }
    }
...
ESConnector.prototype.buildNestedQueries = function (body, model, idName, where) {
    ...
    log('ESConnector.prototype.buildNestedQueries',
        ...
        'model',  model, 'idName', idName, 'where',  JSON.stringify(where,null,0));
    ...
    _.forEach(where, function(value, key) {
        ...
        if (key === 'and' || key === 'or' || key === 'nor') {
            if (Array.isArray(cond)) {
                cond = cond.map(function (c) {
                    self.buildNestedQueries(body, model, idName, c);
                    /*var nestedQuery = self.buildNestedQueries(body, model, idName, c);
                    if(key === 'and'){
                        log('ESConnector.prototype.buildNestedQueries',
                    ...
                    /*if(key === 'or'){
                     should.push(nestedQuery);
                     }
                     if(key === 'nor'){
                     mustNot.push(nestedQuery);
                     }*/
                });
            }
        ...
        if (spec) {
            // TODO: gt, gte - Numerical greater than (>); greater than or equal (>=). Valid only for numerical and date values.
            // TODO: lt, lte - Numerical less than (<); less than or equal (<=). Valid only for numerical and date values.
...
ESConnector.prototype.all = function all(model, filter, done) {
    ...
    if(filter && filter.suggests) { // TODO: remove HACK!!!
        self.db.suggest(
            self.buildFilter(model, idName, filter, 0, 0)
        ).then(
            function (body) {
                var result = [];
                if (body.hits) {
...
ESConnector.prototype.suggest = function find(model, id, done) {
    var self = this;
    if (self.debug) {
        log('find', 'model', model, 'id', id);
    }
...
ESConnector.prototype.count = function count(model, done, filter) {
    ...
    var idName = self.idName(model);
    self.db.count(
        self.buildFilter(model, idName, filter, null, null)
    ).then(
        function (response) {
