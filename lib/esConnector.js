 ...
 */
ESConnector.prototype.buildFilter = function (model, criteria, size, offset) {
    var self = this;
    if (self.debug) {
        log('buildFilter', 'model', model, 'criteria', JSON.stringify(criteria,null,0));
    }
        ...
        else
        {
            if (criteria.where) {
                ...
                _.forEach(criteria.where, function(value, key) {
                    var where = {match:{}};
                    if(key==='id') { // TODO: blindly doing this could cause other issues?
    ...
    log('buildFilter', 'filter', JSON.stringify(filter,null,0));
    return filter;
};
...
ESConnector.prototype.all = function all(model, filter, done) {
    var self = this;
    if (self.debug) {
        log('all', 'model', model, 'filter', JSON.stringify(filter,null,0));
    }

    self.db.search(
        self.buildFilter(model, filter, 0, 0)
    ).then(
        function (body) {
            var result = [];
            ...
            log('all', 'model', model, 'result', JSON.stringify(result,null,2));
            done(null, result);
        }, function (err) {
            console.trace(err.message);
            if (err) {
 ...
 * @param {Function} done callback
 */
ESConnector.prototype.destroyAll = function destroyAll(model, filterClause, done) {
    var self = this;

    if (_.isFunction(filterClause)) {
        done = filterClause;
        filterClause = {
            native: {
                'query': {
    ...
    var filter = self.buildFilter(model, filterClause, 0, 0);
...
ESConnector.prototype.count = function count(model, done, filter) {
    ...
    self.db.count(
        self.buildFilter(model, filter, null, null)
    ).then(
        function (response) {
