...
ESConnector.prototype.connect = function (callback) {
    ...
    if (self.db) {
        ...
        if(self.settings.mappings) {
          self.setupIndices(function(err) {
...
ESConnector.prototype.setupIndices = function (models, callback) {
    var self = this;
    var db = self.db;
    var settings = self.settings;
    ...
    log('ESConnector.prototype.setupIndices', 'models', models);

    models = models || _.pluck(self.settings.mappings, 'name');
    log(models);
    var indices = [];

    _.forEach(models, function (model){
        log('ESConnector.prototype.setupIndices', 'model', model);
    ...
    log('ESConnector.prototype.setupIndices', 'index', indices);
    ...
    .catch(function(err){
        log('ESConnector.prototype.setupIndices', 'failed');
        callback(err);
    });
};
            ...
            Promise.map(
                ...
                function (indexName) {
                    ...
                    return self.db.indices.exists(params)
                      .then(function(exists) {
                        if (exists) {
                          return self.db.indices.delete(params);
                        }
                      })
                      .then(function(res) {
                        return self.db.indices.create(params);
