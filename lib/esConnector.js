...
ESConnector.prototype.getESClientURL = function (settings) {
    ...
    settings.hosts = (settings.hosts || defaultHost);
    ...
    if (settings.ssl) {
      options.ssl = {
        ca: fs.readFileSync( settings.ssl.ca || path.join(__dirname, '..', 'cacert.pem') ),
        rejectUnauthorized: settings.ssl.rejectUnauthorized || true
      };
      //options.host = 'https://' + settings.host + ':' + settings.port;
    }
    console.log(options);
    return options;
};
